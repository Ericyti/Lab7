describe('Basic user flow for Website', () => {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    const cartString = await page.evaluate(() => localStorage.getItem('cart'));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const item of items) {
      ...
      const label = await textHandle.jsonValue();
      expect(label).toBe('Add to Cart');
    }

    const cartCountHandle = await page.$('#cart-count');
    const cartTextHandle = await cartCountHandle.getProperty('innerText');
    const cartValue = await cartTextHandle.jsonValue();
